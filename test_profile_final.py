...
def run_tests():
    ...
    # Verificar JobStaff para job 9
    staff = JobStaff.objects.filter(job_id=9, professional=pro).first()
    if not staff:
        print('Criando JobStaff para teste...')
        from base.jobs.models import Job
        job = Job.objects.get(id=9)
        staff = JobStaff.objects.create(
            job=job,
            professional=pro,
